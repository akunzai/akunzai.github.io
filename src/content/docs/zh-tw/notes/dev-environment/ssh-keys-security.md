...
description: 捨棄 RSA，全面轉向 Ed25519。深入剖析 SSH 密鑰生成、Passphrase 保護、多金鑰管理與 Agent 安全邊界。
sidebar:
  order: 2
---
...
- **安全性評級**：小於 2048 位的 RSA 已經被視為不安全；即便使用 RSA 3072 或 4096 位，其抗暴力破解演算法的性價比也遠不如現代橢圓曲線密碼學。
- **效能與體積**：RSA 4096 位的密鑰長度龐大，加解密運算開銷顯著；而 **Ed25519**（基於 Edwards-curve 25519）金鑰僅有 68 個字元長，計算速度極快且天生免疫旁路攻擊（Side-channel attacks）。
...
- `-o`：強制使用 OpenSSH 新式密鑰格式（相較於舊式 PEM 格式，更具備抗暴力破解機制）。
- `-a 100`：指定密鑰導出函數（KDF，基於 bcrypt）迭代計算 100 次。這會大幅增加攻擊者在私鑰被盜取時使用暴力窮舉密碼的運算成本，而對你日常登入幾乎感覺不到延遲。
...
很多新手看到 `empty for no passphrase`，就像看到免死金牌一樣毫不猶豫地連敲兩次回車留空。心裡想著：*「反正我這只是本機開發，等之後正式上線我再來補密碼」*——這句軟體開發界名言的可靠程度，大概就跟「我下週一開始一定會早起去健身房」差不多。
