...
## 架構演化：站在巨人肩膀上的 Context Engineering

這套體系並非憑空捏造，而是在人機協同的大量實戰踩坑中，前後研讀並揉合了社群與官方的三篇關鍵文獻，演變成一套 Context Engineering：
...
3. **前瞻 Context Engineering：[The New Rules of Context Engineering](https://claude.com/blog/the-new-rules-of-context-engineering-for-claude-5-generation-models)**  
   確立了新一代前瞻模型的 Context Engineering 哲學——**漸進式揭露（Progressive Disclosure）與動態情境掛載**：與其在開場一股腦塞入三千行規則造成注意力稀釋，不如打造常駐骨幹並按需動態喚醒情境規則。
...
2. **符號連結（Symlink）跨工具統整**：不為每個 CLI 重複複製貼上配置。以 `~/.agents/AGENTS.md` 為單一真實來源，再用 symlink 接到各工具會讀的檔案（Copilot CLI 讀 `~/.copilot/copilot-instructions.md`，並不會去讀 `~/.agents/AGENTS.md`）：
   ```bash
...
flowchart TD
    subgraph Governance["Agent 分層治理體系"]
        ...
        T3["<b>◆ Tier 3: 個人 USER.md</b><br/><code>~/.agents/USER.md</code><br/>個人筆記 · Workflow 品味"]
...
### 衝突裁決原則（Precedence）
- **專案規定** 優先於全域手冊的一般通則。
- **個人偏好**（`USER.md`，由 `AGENTS.md` 指標載入，不是各 CLI 原生發現）可覆寫其他一般建議。
...
- **零機敏外洩（Zero Secret / PII Leaks）**：嚴禁以明文形式輸出、轉傳或記錄真實的 API Token、密碼與客戶個資（PII）。
...
許多團隊在導入 Agent 時，習慣把厚達三千行的守則一股腦塞進 Prompt，結果把珍貴的 Context Window 吃得一乾二淨，AI 反而因注意力稀釋而開始胡說八道。

我的解法是**精簡常駐框架，按需動態掛載**：

- **常駐核心（`~/.agents/AGENTS.md`）**：僅保留極簡原則——Precedence、安全底線、記憶持久化規範（明確指示才寫入）、避免重複踩坑（能擋就擋，同一坑只記一處）、時效性外部查證（杜絕憑記憶猜測過期版本號碼）。
- **情境錦囊（`~/.agents/rules/*.md`）**：平時絕不進 Context，唯有觸發特定情境條件時才由 Agent 動態載入：
  ...
  - `subagents.md`：遇到龐大且繁重的探索任務時觸發，**將髒活委派給子 Agent**，在低成本模型中燃燒過程，主線僅接收精煉結論。
...
- [Anthropic: The New Rules of Context Engineering](https://claude.com/blog/the-new-rules-of-context-engineering-for-claude-5-generation-models) — 前瞻模型 Context Engineering 與注意力治理的核心準則
